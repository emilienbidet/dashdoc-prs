...
- ⚠️ **Rate limit risk** — at 100 PRs × (getPr + listReviews + listCheckRuns) that's 300 calls per sync. At 1 sync/min the theoretical max is 18k/hr, over the 5k PAT ceiling. Before shipping the cron live I'll add ETag caching (304 responses don't count) in a dedicated step, or bump the cron interval. For now the code path is wired so we can test it manually via `/api/sync`.
